...
praeorder(e, []).
praeorder(n(Root, Lb, Rb), Ys) :- praeorder(Lb, Ls), praeorder(Rb, Rs), appendl([Root | Ls], Rs, Ys).

% postorder(Xb,Ys) : Ys ist die Liste der Knotenbeschriftungen des Binärbaumes Xb in Postorder.
postorder(e, []).
% die appendls müssen auf die andere seite, je nachdem welche query man stellt (welcher baum erzeugt Xs vs welche postorder kommt durch baum Xb) 
postorder(n(Root, Lb, Rb), Ys) :-  postorder(Lb, Ls), postorder(Rb, Rs), appendl(Rs, [Root], Rroots), appendl(Ls, Rroots, Ys).
