...
wam::query_node wam::make_tree(const wam::executor &top_exec, const std::vector<wam::functor_view>& functors) {
    ...
    std::transform(fact_execs.begin(),
        ...
        //TODO make proceed instruction set executor.success == true, and remove empty executor
        if(exec->failed()){
            ...
            return var_binding_node{exec->get_solved_term_code(),
                                    find_substitutions(
                                            *exec,
                                            functors,
                                            query_var_heap_subs,
